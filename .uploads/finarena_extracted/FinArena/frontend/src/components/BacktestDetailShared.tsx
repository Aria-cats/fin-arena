...
/**
 * 判断 source_url 是否为 Fin Arena 合成的占位链接（点出去一定 404）。
 * 此类链接点击时应当拦截 → 打开「完整事件说明」Modal，而非跳外部 404。
 */
export function isFinArenaSyntheticSourceUrl(url: string): boolean {
  if (!url) return false;
  const FIN_ARENA_DATASET_IDS = [
    "cn_earnings_q2",
    "cn_insiders",
    "cn_policy",
    "us_earnings_q2",
  ...
  const eventIdLike = new RegExp(`(?:${FIN_ARENA_DATASET_IDS.join("|")})-\\d{2,4}`);
  ...
  try {
    ...
    if (host.endsWith("sec.gov") && /-finarena-/.test(path + search)) return true;
    if (host.endsWith("hkexnews.hk") && /finarena_/.test(path + search)) return true;
    if (
      ...
      /(docid|fid|refid|id)=finarena_/.test(search)
    ) {
      return true;
    }
    if (/(finarena_|_finarena_)/.test(url) && !host.includes("127.0.0.1") && !host.includes("localhost")) return true;
    return false;
  } catch {
    return false;
  }
