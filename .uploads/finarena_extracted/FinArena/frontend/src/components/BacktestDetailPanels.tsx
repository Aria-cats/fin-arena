...
import { DirBadge, isFinArenaSyntheticSourceUrl, PreTextBlock } from "./BacktestDetailShared";
...
export function CaseDetailPanel({
  ...
  const synthetic = sourceUrlStr ? isFinArenaSyntheticSourceUrl(sourceUrlStr) : false;
  useEffect(() => {
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          {sourceUrlStr ? (
            <a
              ...
              title={synthetic ? `（Fin Arena 合成数据，点击查看完整原文）${sourceUrlStr}` : sourceUrlStr}
            >
              ...
              {synthetic && (
                <span className="ml-0.5 rounded bg-violet/15 px-1 text-[9.5px] text-violet">Fin Arena 合成</span>
              )}
            </a>
          ) : null}
          <button
            ...
            <div className="space-y-3 px-5 py-4">
              ...
              {sourceUrlStr && (
                <div className="flex flex-wrap items-center gap-2 rounded-md border border-edge/60 bg-edge/20 px-3 py-2 text-[11.5px]">
                  ...
                  {synthetic ? (
                    <>
                      <span className="rounded bg-violet/15 px-1 text-[10px] text-violet">Fin Arena 合成事件</span>
