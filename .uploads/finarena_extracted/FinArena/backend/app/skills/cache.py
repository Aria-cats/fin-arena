...
- 支持 `FINARENA_CACHE_DISABLE=1` 环境变量整库关闭
...
class TTLCache:
    def __init__(self, max_size: int = 1024):
        ...
        self._disabled = os.environ.get("FINARENA_CACHE_DISABLE", "").strip() in ("1", "true", "yes")
...
# ----------------------------------------------------------------- singleton
CACHE = TTLCache(max_size=int(os.environ.get("FINARENA_CACHE_MAX", "1024")))
...
def cached(profile: str = "default"):
    ...
    def deco(fn: Callable[..., dict]) -> Callable[..., dict]:
        ...
        wrapper.__finarena_cache_profile__ = profile  # type: ignore[attr-defined]
        wrapper.__finarena_cache_ttl__ = ttl  # type: ignore[attr-defined]
        return wrapper
